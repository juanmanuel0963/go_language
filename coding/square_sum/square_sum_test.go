...
func Benchmark_judgeSquareSum(b *testing.B) {
	for i := 0; i < b.N; i++ {
		judgeSquareSum(5)
	}
}

func Test_judgeSquareSumVer2(t *testing.T) {
	tests := []struct {
		name string
		c    int
		want bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := judgeSquareSumVer2(tt.c); got != tt.want {
				t.Errorf("judgeSquareSum() = %v, want %v", got, tt.want)
			}
		})
	}
...
func Benchmark_judgeSquareSumVer2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		judgeSquareSumVer2(5)
	}
}
